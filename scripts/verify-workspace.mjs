...
assert.match(desktopPackage.scripts?.['pack:dir'], /scripts\/pack-desktop-directory\.mjs/)
assert.equal(rootPackage.scripts?.['prepare:embedding-model'], 'node scripts/prepare-embedding-model.mjs')
const packDesktopDirectory = await read('scripts/pack-desktop-directory.mjs')
assert.match(packDesktopDirectory, /\['exec', 'electron-builder', '--dir'\]/)
...
assert.match(electronVite, /externalizeDeps:\s*\{/)
assert.match(electronVite, /exclude:\s*\['@memorilo\/editor-storage'\]/)
for (const dependency of ['@huggingface/transformers', 'better-sqlite3', 'sqlite-vec'])
  assert.match(electronVite, new RegExp(`include: \\[.*'${dependency.replace('/', '\\/')}'.*\\]`))
