...
function browserCaptureDocument(documentRef: Document): ReaderCaptureDocument {
  return {
    childDocuments: () => {
      ...
      for (const frame of Array.from(documentRef.querySelectorAll('iframe'))) {
        if (frame.contentDocument)
          childDocuments.push(browserCaptureDocument(frame.contentDocument))
      }
      return childDocuments
    },
    querySelectorAll: selector => Array.from(documentRef.querySelectorAll<HTMLElement | SVGElement>(selector)),
  }
}
