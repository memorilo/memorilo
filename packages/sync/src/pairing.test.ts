...
describe('pairing', () => {
  ...
  it('can validate a server pairing response without retaining it in the global device store', async () => {
    const server = new PairingManager({ deviceId: 'server', deviceName: 'Server', peerId: 'peer-server', role: 'server' }, new MemoryPairingStore(), () => 1_000)
    const client = new PairingManager({ deviceId: 'client', deviceName: 'Client', peerId: 'peer-client' }, new MemoryPairingStore(), () => 1_000)
    await server.load()
    await client.load()

    const accepted = await client.acceptInvitation(server.createInvitation())
    const completed = await server.completeInvitation(accepted.response, { persist: false })

    expect(completed).toMatchObject({ deviceId: 'client', peerId: 'peer-client' })
