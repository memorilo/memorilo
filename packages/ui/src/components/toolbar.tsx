...
function ToolbarRoot({ asChild = false, children, ref, variant = 'plain', xstyle, ...props }: Omit<HTMLAttributes<HTMLDivElement>, 'className' | 'style'> & {
  ...
  return (
    asChild
      ? <Slot {...props} ref={ref} {...stylexRuntime.props(toolbarStyles.root, toolbarStyles[variant], xstyle)} data-ui="toolbar" data-variant={variant} role={props.role ?? 'toolbar'}>{Children.only(children) as ReactElement}</Slot>
