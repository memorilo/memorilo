...
export async function openReaderAdapter(
  ...
  if (resolved.format === 'txt') {
    ...
    return openTxtAdapter({ ...resolved, format: 'txt' }, initialPosition, callbacks)
  }

  if (resolved.format === 'cbz' || resolved.format === 'cbr') {
    const { openComicAdapter } = await import('./comic/comic-adapter')
    return openComicAdapter({ ...resolved, format: resolved.format }, initialPosition, callbacks)
  }

  const { openEpubAdapter } = await import('./epub/epub-adapter')
  return openEpubAdapter({ ...resolved, format: 'epub' }, initialPresentationMode, initialPosition, callbacks)
}
