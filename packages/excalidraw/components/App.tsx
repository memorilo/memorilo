...
class App extends React.Component<AppProps, AppState> {
  ...
  public render() {
    ...
    const { renderToolbarUI, renderTopRightUI, renderCustomStats } = this.props;
    ...
    if (
      ...
      const scaleFactor =
        (this.state.activeTool.type === "freedraw" ||
          this.state.activeTool.type === "marker") &&
        this.state.penMode
          ? 1
  ...
  private handleCanvasPointerDown = (
    ...
    if (
      ...
      this.state.newElement &&
      isFreeDrawElement(this.state.newElement)
    ) {
    ...
    const element =
      elementType === "marker"
        ? newMarkerElement({
            ...
            backgroundColor: this.state.currentItemBackgroundColor,
            fillStyle: this.state.currentItemFillStyle,
            strokeWidth: this.state.currentItemMarkerStrokeWidth,
            strokeStyle: "solid",
            roughness: 0,
            opacity:
              this.state.currentItemMarkerBrush === "highlighter"
                ? Math.min(this.state.currentItemOpacity, 45)
                : this.state.currentItemOpacity,
            brush: this.state.currentItemMarkerBrush,
            blendMode: this.state.currentItemMarkerBlendMode,
            pressureThinning: this.state.currentItemMarkerPressureThinning,
            roundness: null,
            simulatePressure,
            locked: false,
            frameId: topLayerFrame ? topLayerFrame.id : null,
    ...
    return withBatchedUpdatesThrottled((event: PointerEvent) => {
      ...
      if (this.state.selectionElement) {
        ...
        if (isFreeDrawElement(newElement)) {
          ...
          const pressures = [...newElement.pressures];
