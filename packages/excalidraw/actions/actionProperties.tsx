...
const MARKER_BRUSH_DEFAULTS: Record<
  MarkerBrush,
  { blendMode: "normal" | "multiply"; pressureThinning: number }
> = {
  highlighter: { blendMode: "multiply", pressureThinning: 0.08 },
  marker: { blendMode: "normal", pressureThinning: 0.42 },
};
...
export const actionChangeMarkerBrush = register({
  name: "changeMarkerBrush",
  label: "labels.markerBrush",
  trackEvent: false,
  perform: (elements, appState, value) => {
    const brush =
      (value.currentItemMarkerBrush as MarkerBrush | undefined) ??
      appState.currentItemMarkerBrush;
    const defaults = MARKER_BRUSH_DEFAULTS[brush];
    const blendMode =
      value.currentItemMarkerBlendMode ??
      (value.currentItemMarkerBrush
        ? defaults.blendMode
        : appState.currentItemMarkerBlendMode);
    const pressureThinning =
      value.currentItemMarkerPressureThinning ??
      (value.currentItemMarkerBrush
        ? defaults.pressureThinning
        : appState.currentItemMarkerPressureThinning);
    const strokeWidth =
      value.currentItemMarkerStrokeWidth ??
      appState.currentItemMarkerStrokeWidth;
    ...
    return {
      ...
      appState: {
        ...appState,
        ...value,
        currentItemMarkerBrush: brush,
        currentItemMarkerBlendMode: blendMode,
        currentItemMarkerPressureThinning: pressureThinning,
        currentItemMarkerStrokeWidth: strokeWidth,
      },
      captureUpdate: CaptureUpdateAction.IMMEDIATELY,
    };
  },
  PanelComponent: ({ elements, appState, updateData }) => (
    <>
      <fieldset>
        <legend>{t("labels.markerBrush")}</legend>
        <ButtonIconSelect
          group="marker-brush"
          options={[
            {
              value: "highlighter",
              text: t("labels.highlighter"),
              icon: MarkerIcon,
              testId: "marker-brush-highlighter",
            },
            {
              value: "marker",
              text: t("labels.marker"),
              icon: FreedrawIcon,
              testId: "marker-brush-marker",
            },
          ]}
          value={getFormValue(
            elements,
            appState,
            (element) => (element.type === "marker" ? element.brush : null),
            (element) => element.type === "marker",
            appState.currentItemMarkerBrush,
          )}
          onChange={(brush) => {
            const nextBrush = brush as MarkerBrush;
            const defaults = MARKER_BRUSH_DEFAULTS[nextBrush];
            updateData({
              currentItemMarkerBrush: nextBrush,
              currentItemMarkerBlendMode: defaults.blendMode,
              currentItemMarkerPressureThinning: defaults.pressureThinning,
            });
          }}
        />
      </fieldset>
      <fieldset>
        <legend>{t("labels.markerSize")}</legend>
        <ButtonIconSelect
          group="marker-size"
          options={[
            {
              ...
              text: t("labels.thin"),
              icon: StrokeWidthBaseIcon,
              testId: "marker-size-thin",
            },
            {
              value: 10,
              text: t("labels.bold"),
              icon: StrokeWidthBoldIcon,
              testId: "marker-size-bold",
            },
            {
              value: 16,
              text: t("labels.extraBold"),
              icon: StrokeWidthExtraBoldIcon,
              testId: "marker-size-extra-bold",
            },
          ]}
          value={getFormValue(
            elements,
          ...
          onChange={(strokeWidth) =>
            updateData({ currentItemMarkerStrokeWidth: strokeWidth })
          }
        />
      </fieldset>
      <label className="control-label">
        {t("labels.markerPressure")}
        <div className="range-wrapper">
          <input
            ...
            data-testid="marker-pressure"
            onChange={(event) =>
              updateData({
                currentItemMarkerPressureThinning:
