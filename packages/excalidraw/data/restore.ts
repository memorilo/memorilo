...
const restoreElement = (
  ...
  switch (element.type) {
    ...
    case "marker": {
      const brush = element.brush === "marker" ? "marker" : "highlighter";
      return restoreElementWithProperties(element, {
        points: element.points,
        lastCommittedPoint: null,
        simulatePressure: element.simulatePressure,
        ...
        blendMode:
          element.blendMode === "normal"
            ...
            : "multiply",
        pressureThinning:
          typeof element.pressureThinning === "number" &&
          Number.isFinite(element.pressureThinning)
            ? Math.min(Math.max(element.pressureThinning, 0), 1)
            : brush === "marker"
            ? 0.42
            : 0.08,
      });
