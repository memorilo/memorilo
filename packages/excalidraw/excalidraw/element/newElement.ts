...
export const newMarkerElement = (
  opts: {
    type: "marker";
    points?: ExcalidrawMarkerElement["points"];
    simulatePressure: boolean;
    pressures?: ExcalidrawMarkerElement["pressures"];
    brush?: ExcalidrawMarkerElement["brush"];
    blendMode?: ExcalidrawMarkerElement["blendMode"];
    pressureThinning?: ExcalidrawMarkerElement["pressureThinning"];
  } & ElementConstructorOpts,
): NonDeleted<ExcalidrawMarkerElement> => {
  return {
    ..._newElementBase<ExcalidrawMarkerElement>(opts.type, opts),
    points: opts.points || [],
    pressures: opts.pressures || [],
    simulatePressure: opts.simulatePressure,
    lastCommittedPoint: null,
    brush: opts.brush ?? "highlighter",
    blendMode: opts.blendMode ?? "multiply",
    pressureThinning: opts.pressureThinning ?? 0.08,
  };
};
