    ...
    case "marker": {
      ...
      if (element.type === "marker") {
        context.globalCompositeOperation =
          element.blendMode === "multiply" ? "multiply" : "source-over";
      }
...
type InkElement = ExcalidrawFreeDrawElement | ExcalidrawMarkerElement;

export const pathsCache = new WeakMap<InkElement, Path2D>([]);

export function generateFreeDrawShape(element: InkElement) {
...
export function getFreeDrawPath2D(element: InkElement) {
  return pathsCache.get(element);
}

export function getFreeDrawSvgPath(element: InkElement) {
  ...
  const options: StrokeOptions = {
    ...
    thinning: element.type === "marker" ? element.pressureThinning : 0.6,
    smoothing: element.type === "marker" ? 0.35 : 0.5,
    streamline: element.type === "marker" ? 0.35 : 0.5,
