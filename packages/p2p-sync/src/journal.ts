import type { DeviceId, SyncChange, VersionVector } from './model'
...
export class JsonSyncJournal {
  ...
  getVersionVector(): VersionVector {
    ...
    // Local changes are already authoritative on this device. Remote changes
    // only advance the vector through `receivedVersionVector`, which tracks
    // contiguous sequences and prevents advertising a gap as synchronized.
  ...
  listChanges(since: VersionVector): readonly SyncChange[] {
    this.#assertLoaded()
    const normalized = normalizeVersionVector(since)
    return this.#state.changes
      .filter(change => change.sequence > (normalized[change.deviceId] ?? 0))
      .map(change => ({ ...change }))
