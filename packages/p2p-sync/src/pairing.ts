import type { DeviceId, PairedDevice, PairingInvitation, PairingResponse } from './model'
...
export interface PairingStore {
  load: () => Promise<readonly PairedDevice[]>
  save: (devices: readonly PairedDevice[]) => Promise<void>
}
...
export class PairingManager {
  ...
  createInvitation(ttlMs = 10 * 60 * 1000): string {
    ...
    const invitation: PairingInvitation = {
      ...
      membershipEpoch: validMembershipEpoch(this.identity.membershipEpoch),
      createdAt: now,
      expiresAt: now + ttlMs,
    }
  ...
  async acceptInvitation(encoded: string): Promise<{ response: string, device: PairedDevice }> {
    ...
    const device = this.makeDevice(invitation.pairingId, invitation.deviceId, invitation.deviceName, invitation.peerId, invitation.sharedSecret)
  ...
  async completeInvitation(encoded: string): Promise<PairedDevice> {
    const response = decodePairingPayload<PairingResponse>(encoded)
    if (!isResponse(response) || response.deviceId === this.identity.deviceId)
      throw new TypeError('Pairing response is incomplete')
    return this.upsert(this.makeDevice(response.pairingId, response.deviceId, response.deviceName, response.peerId, response.sharedSecret))
  }
  ...
  async completeGrant(grant: PairingGrant): Promise<PairedDevice> {
    if (!isNonEmptyString(grant.pairingId) || !isNonEmptyString(grant.deviceId)
      ...
      || !isNonEmptyString(grant.sharedSecret)) {
  ...
  private makeDevice(pairingId: string, deviceId: string, deviceName: string, peerId: string, sharedSecret: string): PairedDevice {
    return {
      addedAt: this.now(),
      deviceId,
