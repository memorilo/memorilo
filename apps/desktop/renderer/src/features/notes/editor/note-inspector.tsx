...
export function NoteInspector({
  ...
  noteId: string
  onOpenBookContextMenu: (event: ReactMouseEvent, topicId: string, readingId: string) => void
  onOpenContainerContextMenu: (event: ReactMouseEvent, parentId: string | null, allowFolder: boolean) => void
    ...
    <AnimatePresence initial={false}>
      {open
        ? (
            ...
            >
              <header {...stylex.props(noteInspectorStyles.inspectorTitlebar)}>
                <div {...stylex.props(noteInspectorStyles.inspectorTitleGroup)}>
                  <h1 {...stylex.props(noteInspectorStyles.inspectorTitle)}>{t('topics')}</h1>
              ...
              <div {...stylex.props(noteInspectorStyles.inspectorContent)}>
                <section {...stylex.props(noteInspectorStyles.inspectorSection)} aria-labelledby="topic-structure-heading">
                  <div
                    {...stylex.props(noteInspectorStyles.inspectorSectionHeading)}
                    onContextMenu={event => onOpenContainerContextMenu(event, null, true)}
                  >
                    <h2 id="topic-structure-heading" {...stylex.props(noteInspectorStyles.inspectorSectionTitle)}>
                      {t('structure')}
                    </h2>
                  </div>
                      ...
                      {visibleEntries.map(({ depth, entry, hasChildren }) => {
                        ...
                        const topicContextMenu = entry.kind === 'topic'
                          ? entry.topicType === 'book'
                            ? (event: ReactMouseEvent) => onOpenBookContextMenu(
                                event,
                                entry.id,
                                bookTopicReadingId(entry),
                              )
                            : (event: ReactMouseEvent) => onOpenContainerContextMenu(event, entry.id, false)
                          : undefined
                            ...
                            {entry.kind === 'folder'
                              ? (
                                  <button
                                    ...
                                    onContextMenu={event => onOpenContainerContextMenu(event, entry.id, true)}
                                  >
                                    <motion.span
