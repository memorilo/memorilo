...
An acknowledgement may contain no mutation IDs when the server only advances the watermark. In that case the client still advances `last_server_sequence` and retains all pending outbox mutations for later acknowledgement.
