...
- Earlier Android shell-only emulator checks covered Notes, Journal, Learning, and Shelf navigation. The Android API 35 AVD now also verifies the full native Loro/Hermes shell and rich Editor with the tracked `loro-react-native@1.10.3` patch. The managed reading library avoids Hermes-incompatible `toSorted`/`with` collection helpers.
...
- A clean iOS Release build was verified through Xcode with the generated `apps/mobile/ios/Memorilo.xcworkspace` on iOS 26.5, using the iPhone 17 simulator destination. The build completed the native Hermes bundle plus Editor, Reader, Learning, and FSRS DOM surfaces with `** BUILD SUCCEEDED **`; the resulting `Memorilo.app` is a signed simulator bundle containing arm64 and x86_64 binaries.
- The iOS Release app was installed and cold-launched offline on one temporary iPhone 17 simulator without opening the Simulator GUI. `simctl launch` returned a live `com.memorilo.app` process; the fresh app database passed `PRAGMA integrity_check = ok`, reported `user_version = 1`, created FTS5 trigram tables and `topic_block_embeddings` with `vec0 FLOAT[384]`, and its normalized schema inspection matched the canonical artifact byte-for-byte at SHA-256 `9ced1dabf6d543697278f591d4846312756c1a62045df598462796751dd96913`. The temporary simulator was shut down and deleted immediately after this check; no Booted device or Simulator GUI process remains.
- Release-only strict dependency resolution was closed for the mobile build: `packages/srs` now directly declares `@open-spaced-repetition/binding-wasm32-wasi`, `apps/mobile` directly declares `babel-preset-expo` and `sass`, and Metro resolves the FSRS WASI assets from the SRS package owner. This removes reliance on pnpm-transitive visibility and makes the Xcode offline bundle reproducible.
- Android `loro-react-native@1.10.3` compatibility with React Native `0.86.2` is now tracked in `patches/loro-react-native@1.10.3.patch` and `pnpm-workspace.yaml`. The patch replaces the obsolete reflective `CallInvokerHolderImpl.mHybridData` access with the RN 0.86 `CallInvokerHolder::javaobject`/`cthis()` API; its SHA-256 is `1031ddf40075c3c858078b59e4d091890d9b702f1e3d6d422cfb6e1b099a72a0`.
- With that patch applied, `rtk nix develop .#android --command ./gradlew :app:assembleDebug --rerun-tasks` completed successfully across all Android ABIs in 5m14s. The resulting 360 MiB APK installed on the repository `memorilo-api35` AVD and `com.memorilo.app/.MainActivity` cold-started with a live process; filtered logcat contained no `NoSuchFieldError`, `SIGABRT`, JNI abort, `FATAL EXCEPTION`, RedBox, or Jotai error.
- A workspace regression pass after the Reader work completed `pnpm lint` (37 tasks), `pnpm typecheck` (37 tasks), and `pnpm test` (35 tasks). The Editor suite passed 31 files/325 tests, the Reader suite passed 28 files/131 tests, and the desktop main/renderer suites passed without failures.
- Desktop production build completed through `pnpm turbo run build --filter=@memorilo/desktop...`; Electron E2E completed all 35 tests with `MEMORILO_E2E_HIDE_WINDOW=1`. The only build messages were existing Excalidraw dynamic-import and electron-builder metadata warnings.
- Workspace verification exposed and fixed a shared-package configuration gap: `packages/application/tsconfig.json` now enables `allowImportingTsExtensions`, matching the existing Editor configuration and allowing the extracted application package to typecheck when Turbo builds its Editor dependency.
- Fresh schema parity was verified from a live Android API 35 database and a newly created desktop database using the same 384-dimensional `Xenova/multilingual-e5-small` model metadata. Both normalized inspection artifacts are byte-identical with SHA-256 `9ced1dabf6d543697278f591d4846312756c1a62045df598462796751dd96913`; the Android copy had `PRAGMA integrity_check = ok`, `user_version = 1`, learning schema generation `1`, and `vec0 FLOAT[384]` plus FTS5 trigram virtual tables. The Android AVD was shut down immediately after this read-only check.
- Mobile source audit found no imports from `apps/desktop` private source paths. Editor, Reader, application, shelf, reading-model, and storage usage resolves through their declared public package entry points. Generated native projects, export bundles, embedding assets, desktop output, and temporary verification files remain ignored or outside the repository.
...
- Cross-platform schema dump comparison artifacts for a fresh real desktop database and a fresh real native mobile database, plus final repository/device verification. The fresh iOS Release database now has canonical parity evidence; the existing historical iOS simulator container is still not treated as fresh parity evidence because removing its cache-only tables would be a destructive compatibility decision.
- Android release startup is no longer blocked by the published `loro-react-native@1.10.3` adapter: the minimal tracked patch above supplies the React Native `0.86.2` JNI access pattern. Native Reader formats, SQLite capability checks, and physical-device acceptance remain separate and incomplete.
- A read-only registry/source audit confirms that `1.10.3` remains the latest published `loro-react-native` version and that `loro-crdt/base64` instantiates WebAssembly; it cannot replace the native Hermes binding. The repository therefore keeps the minimal pnpm patch instead of switching to an unpublished Git dependency.
